...
| `FLEXKV_ENABLE_CE_MEMCPY2D` | bool | 1 | `SEGMENT_SCATTER` / `GATHER_SCATTER` / `GATHER_DIRECT` path selection (effective for both D2H and H2D). `1` = **Default**, uses `cudaMemcpy2DAsync` for strided direct transfer (~58ms on NVIDIA H20); `0` = uses staging buffer + CPU scatter/gather (works on all platforms; must be 0 on P800/Kunlunxin, ~200× worse there). Only effective when CE transfer is enabled and path 2 `SEGMENT_SCATTER`, path 3 `GATHER_SCATTER`, or path 4 `GATHER_DIRECT` is selected; other paths (CONTIG_DIRECT / SEGMENT_DIRECT) are unaffected |
