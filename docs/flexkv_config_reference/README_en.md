...
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | STAGED_SCATTER D2H path selection. `0` = **Default**, uses staging buffer + CPU scatter (works on all platforms); `1` = uses `cudaMemcpy2DAsync` for strided D2H directly to CPU positions (~58ms on NVIDIA H20, but extremely slow 12.8s on P800/Kunlunxin). Effective when CE transfer is enabled and STAGED_SCATTER is selected |
