...
| `FLEXKV_TRANSFER_PINGPONG` | bool | 1 | Enable ping-pong double-buffered overlap for `STAGED_SCATTER` / `GATHER_SCATTER` staging transfers (BLOCKFIRST layout). When enabled, GPU transfer of layer L+1 overlaps with CPU scatter/gather of layer L, hiding CPU memcpy latency. No-op for `BULK_CONTIG` / `SEGMENTED_DIRECT` (no staging). Disable for debugging or when CPU is saturated |
