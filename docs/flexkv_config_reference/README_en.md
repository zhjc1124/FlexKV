...
| `FLEXKV_ENABLE_CE_MEMCPY_BATCH` | bool | 0 | Controls `cudaMemcpyBatchAsync` batched submission (CUDA 12.0+). `1` = coalesces the N individual `cudaMemcpy*Async` driver submissions within one transfer into a single batched submission — the PER_BLOCK path and the non-2D branches of the five adaptive paths use the 1D-array variant, while the `cudaMemcpy2DAsync` branch uses the 2D `cudaMemcpyParams` variant. This **only reduces host/driver submission overhead**; the actual GPU copy byte count is identical to per-call submission. `0` = **default**: keeps per-call submission (existing behavior). Fully orthogonal to `FLEXKV_ENABLE_CE_MEMCPY2D` / `FLEXKV_CE_PATH_OPT` and freely combinable (covers three combinations: ① PER_BLOCK+batch ② ce_opt+memcpy2d off+batch ③ ce_opt+memcpy2d on+batch). On P800 / CUDA<12 build targets the batch code block is compile-time excluded and silently falls back to per-call submission, with no link or runtime errors. Ships OFF by default for compatibility and zero regression; flip to ON in a separate change after validating the benefit on a real NVIDIA machine |
