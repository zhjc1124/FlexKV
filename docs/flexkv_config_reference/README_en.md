...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | Enable the CE optimized four-path adaptive strategies. `0` = baseline (`PER_BLOCK` per-block memcpy, no optimization, for benchmarking); `1` = optimized paths (`BULK_CONTIG` / `SEGMENTED_DIRECT` / `STAGED_SCATTER` / `GATHER_SCATTER` auto-select, **default**). Only effective when `FLEXKV_USE_CE_TRANSFER_H2D` or `FLEXKV_USE_CE_TRANSFER_D2H` is 1 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | Segment count threshold for `STAGED_SCATTER` vs `GATHER_SCATTER` selection (only when `FLEXKV_TRANSFER_PATH_OPT=1`). When the number of non-contiguous segments ≤ threshold, `STAGED_SCATTER` (staging + CPU scatter) is used; otherwise `GATHER_SCATTER` (GPU index_select/index_copy_ pipeline). Use `microbenchmark_ce_threshold.py` to find the optimal value for your workload |
| `FLEXKV_MLA_D2H_MODE` | str | "auto" | **Only applicable to MLA scenarios** (kv_heads=1, all TP ranks have identical KV). Controls how CPU KV Cache is written during D2H transfer. Available options:<br/>• `auto` - **Default**. Auto-select: `sharded` when CE transfer is off, `rank0_only` when CE transfer is on (rank0_only gives contiguous GPU memory access pattern that CE multi-path strategy exploits)<br/>• `sharded` - Each GPU writes 1/N shard to form one complete KV (requires `chunk_size % num_gpus == 0`)<br/>• `all_write` - Each GPU writes complete KV to its own location (N× CPU memory)<br/>• `rank0_only` - Only rank 0 writes complete KV (best with CE for contiguous memcpy) |
| `FLEXKV_ENABLE_SHARDED_MLA_D2H_MEMCPY2D` | bool | 0 | Sharded MLA BLOCKFIRST D2H path selection. `0` = **Default**, uses D2D shard transpose + contiguous D2H + CPU interleave merge (works on all platforms, ~130ms on P800); `1` = uses `cudaMemcpy2DAsync` for strided D2H directly to CPU interleave (~58ms on NVIDIA H20, but extremely slow 12.8s on P800/Kunlunxin). Only effective when `FLEXKV_MLA_D2H_MODE=sharded` and BLOCKFIRST layout and CE enabled |
