...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | Enable the CE optimized six-path adaptive strategy. `0` = baseline (`PER_BLOCK` per-block memcpy, no optimization, for benchmarking); `1` = optimized paths (`BULK_CONTIG` / `SEGMENTED_DIRECT` / `STAGED_MERGE` / `STAGED_BLOCK` / `GATHER_SCATTER` / `BF_TRANSPOSE` auto-select, **default**). Only effective when `FLEXKV_USE_CE_TRANSFER_H2D` or `FLEXKV_USE_CE_TRANSFER_D2H` is 1 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | Segment count threshold for `SEGMENTED_DIRECT` vs `GATHER_SCATTER` selection (only when `FLEXKV_TRANSFER_PATH_OPT=1`). **Only effective for LAYERFIRST scenarios** (non-contiguous segments ≤ threshold → `SEGMENTED_DIRECT`, otherwise → `GATHER_SCATTER`). BLOCKFIRST scenarios always use `BF_TRANSPOSE` and are not affected by threshold. Use `microbenchmark_ce_threshold.py` to find the optimal value for your workload |
...
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | STAGED_MERGE D2H path selection. `0` = **Default**, uses staging buffer + CPU scatter (works on all platforms); `1` = uses `cudaMemcpy2DAsync` for strided D2H directly to CPU positions (~58ms on NVIDIA H20, but extremely slow 12.8s on P800/Kunlunxin). Effective when CE transfer is enabled and STAGED_MERGE is selected (STAGED_BLOCK no longer uses memcpy2d — shard_size is too small for benefit) |
