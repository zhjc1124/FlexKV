...
| `FLEXKV_ENABLE_CE_MEMCPY_BATCH` | bool | 0 | 控制 `cudaMemcpyBatchAsync` 合并提交（CUDA 12.0+）。`1` = 将一次传输中原本 N 次 `cudaMemcpy*Async` 的 driver 提交合并为 1 次（PER_BLOCK 与五条自适应路径的非 2D 分支使用 1D 数组版；`cudaMemcpy2DAsync` 分支使用 2D `cudaMemcpyParams` 版），**仅减少 host/driver 提交开销**，GPU 实际拷贝字节数与逐笔完全一致；`0` = **默认**：保持逐笔提交（现有行为）。与 `FLEXKV_ENABLE_CE_MEMCPY2D` / `FLEXKV_CE_PATH_OPT` 完全正交，可任意组合（覆盖三组合：① PER_BLOCK+batch ② ce_opt+memcpy2d 关+batch ③ ce_opt+memcpy2d 开+batch）。P800 / CUDA<12 编译目标在编译期剔除 batch 代码块、静默降级为逐笔提交，链接与运行均不报错。首版默认 OFF 以保兼容、零回归，待 NVIDIA 真机验证收益后再单独翻为 ON |
