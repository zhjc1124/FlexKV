...
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | `SEGMENT_DIRECT` 与 `GATHER_SCATTER` 的段数切换阈值（仅 `FLEXKV_TRANSFER_PATH_OPT=1` 时生效）。**仅对 LAYERFIRST 场景生效**（非连续段数 ≤ 阈值时走 `SEGMENT_DIRECT`，否则走 `GATHER_SCATTER`）。BLOCKFIRST 场景始终走 `GATHER_DIRECT`，不受 threshold 影响。可使用 `microbenchmark_ce_threshold.py` 找到工作负载的最优值 |
...
| `FLEXKV_ENABLE_CE_MEMCPY2D` | bool | 1 | `SEGMENT_SCATTER` / `GATHER_SCATTER` / `GATHER_DIRECT` 路径选择（D2H 与 H2D 双向生效）。`1` = **默认**，使用 `cudaMemcpy2DAsync` 做 strided 直传（NVIDIA H20 上约 58ms）；`0` = 使用 staging buffer + CPU scatter/gather（兼容所有平台，P800/Kunlunxin 上必须设为 0，否则约 200× 劣化 12.8s）。仅在 CE 传输开启且选择路径 2 `SEGMENT_SCATTER`、路径 3 `GATHER_SCATTER` 或路径 4 `GATHER_DIRECT` 时生效；其余路径（`CONTIG_DIRECT` / `SEGMENT_DIRECT`）不受影响 |
