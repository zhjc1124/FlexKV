...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | 是否启用 CE 优化路径（六路自适应策略）。`0` = baseline（`PER_BLOCK` 逐块 memcpy，无优化，用于性能对照基准）；`1` = 优化路径（`BULK_CONTIG` / `SEGMENTED_DIRECT` / `STAGED_MERGE` / `STAGED_BLOCK` / `GATHER_SCATTER` / `BF_TRANSPOSE` 自动选择，**默认**）。仅在 `FLEXKV_USE_CE_TRANSFER_H2D` 或 `FLEXKV_USE_CE_TRANSFER_D2H` 为 1 时生效 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | `SEGMENTED_DIRECT` 与 `GATHER_SCATTER` 的段数切换阈值（仅 `FLEXKV_TRANSFER_PATH_OPT=1` 时生效）。**仅对 LAYERFIRST 场景生效**（非连续段数 ≤ 阈值时走 `SEGMENTED_DIRECT`，否则走 `GATHER_SCATTER`）。BLOCKFIRST 场景始终走 `BF_TRANSPOSE`，不受 threshold 影响。可使用 `microbenchmark_ce_threshold.py` 找到工作负载的最优值 |
| `FLEXKV_MLA_D2H_MODE` | str | "rank_rotate" | **仅适用于 MLA 场景**（kv_heads=1，所有 TP rank 的 KV 相同）。控制 D2H 时 CPU KV Cache 的写入模式。可选值：<br/>• `rank_rotate` - **默认**。跨请求轮换 designated rank 写完整 KV，避免单 rank 持续满载导致热节流，配合 CE 效果最佳<br/>• `sharded` - 每个 GPU 写 1/N 分片拼成一个完整 KV（需要 `chunk_size % num_gpus == 0`）<br/>• `all_write` - 每个 GPU 写完整 KV 到各自位置（CPU 内存占用 N×）<br/>• `rank0_only` - 仅 rank 0 写完整 KV<br/>• `layer_parallel` - 按 layer 并行写入 |
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | STAGED_MERGE D2H 路径选择。`0` = **默认**，使用 staging buffer + CPU scatter（兼容所有平台）；`1` = 使用 `cudaMemcpy2DAsync` 做 strided D2H 直写 CPU 位置（NVIDIA H20 上约 58ms，但 P800/Kunlunxin 上极慢 12.8s）。在 CE 传输开启且选择 STAGED_MERGE 路径时生效（STAGED_BLOCK 已去掉 memcpy2d，shard_size 太小无收益） |
