...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | 是否启用 CE 优化路径（四路自适应策略）。`0` = baseline（`PER_BLOCK` 逐块 memcpy，无优化，用于性能对照基准）；`1` = 优化路径（`BULK_CONTIG` / `SEGMENTED_DIRECT` / `STAGED_SCATTER` / `GATHER_SCATTER` 自动选择，**默认**）。仅在 `FLEXKV_USE_CE_TRANSFER_H2D` 或 `FLEXKV_USE_CE_TRANSFER_D2H` 为 1 时生效 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | `STAGED_SCATTER` 与 `GATHER_SCATTER` 的段数切换阈值（仅 `FLEXKV_TRANSFER_PATH_OPT=1` 时生效）。非连续段数 ≤ 阈值时走 `STAGED_SCATTER`（staging + CPU scatter），否则走 `GATHER_SCATTER`（GPU index_select/index_copy_ pipeline）。可使用 `microbenchmark_ce_threshold.py` 找到工作负载的最优值 |
| `FLEXKV_MLA_D2H_MODE` | str | "auto" | **仅适用于 MLA 场景**（kv_heads=1，所有 TP rank 的 KV 相同）。控制 D2H 时 CPU KV Cache 的写入模式。可选值：<br/>• `auto` - **默认**。自动选择：CE 关闭时用 `sharded`，CE 开启时用 `rank0_only`（rank0_only 提供 CE 多路径策略可利用的连续 GPU 内存访问模式）<br/>• `sharded` - 每个 GPU 写 1/N 分片拼成一个完整 KV（需要 `chunk_size % num_gpus == 0`）<br/>• `all_write` - 每个 GPU 写完整 KV 到各自位置（CPU 内存占用 N×）<br/>• `rank0_only` - 仅 rank 0 写完整 KV（配合 CE 效果最佳） |
| `FLEXKV_ENABLE_SHARDED_MLA_D2H_MEMCPY2D` | bool | 0 | sharded MLA BLOCKFIRST D2H 路径选择。`0` = **默认**，使用 D2D shard 转置 + 连续 D2H + CPU interleave merge（兼容所有平台，P800 上约 130ms）；`1` = 使用 `cudaMemcpy2DAsync` 做 strided D2H 直写 CPU interleave（NVIDIA H20 上约 58ms，但 P800/Kunlunxin 上极慢 12.8s）。仅在 `FLEXKV_MLA_D2H_MODE=sharded` 且 BLOCKFIRST 布局且 CE 开启时生效 |
