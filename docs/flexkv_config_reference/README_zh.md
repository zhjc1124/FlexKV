...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | 是否启用 CE 优化路径（五路自适应策略）。`0` = baseline（`PER_BLOCK` 逐块 memcpy，无优化，用于性能对照基准）；`1` = 优化路径（`BULK_CONTIG` / `SEGMENTED_DIRECT` / `STAGED_MERGE` / `STAGED_BLOCK` / `GATHER_SCATTER` 自动选择，**默认**）。仅在 `FLEXKV_USE_CE_TRANSFER_H2D` 或 `FLEXKV_USE_CE_TRANSFER_D2H` 为 1 时生效 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | `STAGED_MERGE`/`STAGED_BLOCK` 与 `GATHER_SCATTER` 的段数切换阈值（仅 `FLEXKV_TRANSFER_PATH_OPT=1` 时生效）。非连续段数 ≤ 阈值时走 `STAGED_MERGE`/`STAGED_BLOCK`（staging + CPU scatter），否则走 `GATHER_SCATTER`（GPU index_select/index_copy_ pipeline）。可使用 `microbenchmark_ce_threshold.py` 找到工作负载的最优值 |
...
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | STAGED_MERGE/STAGED_BLOCK D2H 路径选择。`0` = **默认**，使用 staging buffer + CPU scatter（兼容所有平台）；`1` = 使用 `cudaMemcpy2DAsync` 做 strided D2H 直写 CPU 位置（NVIDIA H20 上约 58ms，但 P800/Kunlunxin 上极慢 12.8s）。在 CE 传输开启且选择 STAGED_MERGE/STAGED_BLOCK 路径时生效 |
