...
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | `SEGMENT_SCATTER` / `GATHER_DIRECT` 路径选择（D2H 与 H2D 双向生效）。`0` = **默认**，使用 staging buffer + CPU scatter/gather（兼容所有平台，P800 上性能最佳）；`1` = 使用 `cudaMemcpy2DAsync` 做 strided 直传（NVIDIA H20 上约 58ms，但 P800/Kunlunxin 上极慢 12.8s，约 200× 劣化）。仅在 CE 传输开启且选择路径 2 `SEGMENT_SCATTER` 或路径 4 `GATHER_DIRECT` 时生效；其余路径（`CONTIG_DIRECT` / `SEGMENT_DIRECT` / `GATHER_SCATTER`）不受影响 |
