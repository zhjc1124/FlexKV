...
| `FLEXKV_ENABLE_MEMCPY2D` | bool | 0 | STAGED_SCATTER D2H 路径选择。`0` = **默认**，使用 staging buffer + CPU scatter（兼容所有平台）；`1` = 使用 `cudaMemcpy2DAsync` 做 strided D2H 直写 CPU 位置（NVIDIA H20 上约 58ms，但 P800/Kunlunxin 上极慢 12.8s）。在 CE 传输开启且选择 STAGED_SCATTER 路径时生效 |
