...
| `FLEXKV_TRANSFER_PINGPONG` | bool | 1 | 是否为 `STAGED_SCATTER` / `GATHER_SCATTER` 的 staging 传输启用 ping-pong 双缓冲重叠（BLOCKFIRST 布局）。启用后 GPU 传输第 L+1 层与 CPU scatter/gather 第 L 层重叠执行，隐藏 CPU memcpy 延迟。对 `BULK_CONTIG` / `SEGMENTED_DIRECT`（无 staging）是 no-op。调试或 CPU 已饱和时可关闭 |
